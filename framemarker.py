...
def process(frame):
    ...
    contours = [contour for contour in contours if
                cv2.contourArea(contour) > 3000.0]
...
def video():
    ...
    # out = cv2.VideoWriter('outvideo1sp.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10, (frame_width,frame_height))
    frame_count = 0
    while True:
    ...
    print(f"% Object True / frame_counts: {(frame_count - COUNT[0]) / frame_count}")
    # cv2.waitKey(0)
...
def imgg():
    ...
    image = cv2.imread("Data\\image\\2.jpg")
    # width = 480
    # height = 640
    # dim = (width, height)
    # # resize image
