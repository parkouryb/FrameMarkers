...
dt = {"38-218-264-335": MARK_IMAGE_2,
      "39-219-265-334": MARK_IMAGE_3,
      "37-217-267-332": MARK_IMAGE_4,
      "36-216-266-333": MARK_IMAGE_5}
...
def edge_process(image):
    ...
    # print(border.shape)
    c = 0
    for k in range(17, border.shape[1], 44):
        if np.sum(border[0:k, k:min(k + 24, border.shape[1])]) / 255 > area * 0.5:
            code += "0"
        else:
            code += "1"
        c += 1
...
def marker_process(image, index):
    ...
    mats = [edge_process(cv2.rotate(img[0:size, 0:20], cv2.ROTATE_90_CLOCKWISE)),
            edge_process(img[0:20, 0:size]),
            edge_process(cv2.rotate(img[0:size, size - 20:size], cv2.ROTATE_90_COUNTERCLOCKWISE)),
            edge_process(cv2.rotate(img[size - 20:size, 0:size], cv2.ROTATE_180))]
    mats = [str(binaryToDecimal(int(mats[i]))) for i in range(len(mats)) if mats[i] != "*"]
    # cv2.imshow("bug" + str(index), cv2.rotate(img[0:size, size - 20:size], cv2.ROTATE_90_COUNTERCLOCKWISE))
    if len(mats) == 0 or mats[0] == '0':
        return "", "", threshold
    # print("mats", mats)

    key = ""
    position = ""
    bb = [['LTRB', 'TRBL', 'RBLT', 'BLTR'],
          ['BLTR', 'LTRB', 'TRBL', 'RBLT'],
          ['RBLT', 'BLTR', 'LTRB', 'TRBL'],
          ['TRBL', 'RBLT', 'BLTR', 'LTRB']]
